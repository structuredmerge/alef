...
/// Regression test for the rc.13/rc.14 incident (now targeting `cmd/setup`, the
/// `cmd/download_ffi` successor): `sync-versions` must rewrite `moduleVersion = "..."`
/// in `packages/go/cmd/setup/main.go` and the `RequireNativeSetup_<ident>` sentinel in
/// `packages/go/native_setup.go` so that Go module consumers of a freshly released
/// version download the correct native library and the version-skew compile guard
/// stays consistent with it.
///
/// Without this fix, the developer's `alef all` (run before the version bump) bakes the
/// old version into these files; the subsequent `sync-versions` bump updated Cargo.toml
...
fn sync_versions_updates_go_module_version_in_cmd_setup() {
    ...
    let setup_dir = root.join("packages/go/cmd/setup");
    std::fs::create_dir_all(&setup_dir).expect("mkdir cmd/setup");
    let stale_main_go = concat!(
        "// cmd/setup downloads the platform-specific native library from GitHub releases.\n",
        "package main\n\nconst (\n",
    ...
    let go_pkg_dir = root.join("packages/go");
    let stale_native_setup_go = concat!(
        "// Code generated by alef — DO NOT EDIT.\n",
        "package mylib\n\n",
        "const RequireNativeSetup_1_9_0_rc_13 = \"1.9.0-rc.13\"\n",
    );
    std::fs::write(go_pkg_dir.join("native_setup.go"), stale_native_setup_go).expect("write native_setup.go");
    ...
    let updated_main = std::fs::read_to_string(setup_dir.join("main.go")).expect("read main.go");
    assert!(
    ...
    let updated_native_setup =
        std::fs::read_to_string(go_pkg_dir.join("native_setup.go")).expect("read native_setup.go");
    assert!(
        updated_native_setup.contains("const RequireNativeSetup_1_9_0_rc_14 = \"1.9.0-rc.14\""),
        "sentinel must be updated to the rc.14 identifier and value:\n{updated_native_setup}"
    );
    assert!(
